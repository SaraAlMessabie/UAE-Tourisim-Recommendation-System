...
def render_listing_card(listing: dict, listing_type: str):
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            details = []
            for key in ["Location", "location", "Price_Range", "price_range",
                        "Environment", "environment", "Start_Date", "End_Date",
                        "Avg_Rating", "avg_rating"]:
                if key in listing and listing[key] not in (None, "", "nan"):
                    details.append(f"**{key.replace('_', ' ')}:** {listing[key]}")
            if details:
